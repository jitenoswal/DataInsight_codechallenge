...
def input_file(input_path):
	...
	df = pd.read_csv(input_path, sep=r'\s* - - \[(\d\d/\w{3}/\d{4}:\d{2}:\d{2}:\d{2}) (-\d{4})\] \"(.*)\" (.*) ', parse_dates=['date'], infer_datetime_format=True,engine='python', names=['host', 'date', 'tzone', 'resource', 'http', 'byte'])
